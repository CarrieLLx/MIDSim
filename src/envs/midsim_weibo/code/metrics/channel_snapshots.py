...
def _merge_blog_ids(prev_ids: Any, new_ids: List[str]) -> List[str]:
...
def record_recommendations_by_source_step(
    ...
    """Append blog ids to profile.recommended_blog_ids_by_channel for this source + step."""
    if profile is None:
        return
    ...
    raw_root = profile.get_data("recommended_blog_ids_by_channel", {})
    ...
    merged = _merge_blog_ids(step_map.get(step_key), list(recommendations.keys()))
    step_map[step_key] = merged
    by_ch[st] = step_map
    profile.update_data("recommended_blog_ids_by_channel", by_ch)
...
def record_mentioned_blog_ids_by_channel(
    profile: Any,
    profile_id: Any,
    current_step: int,
    ...
    """Append blog ids to profile.mentioned_blog_ids_by_channel for this mention type + step."""
    ...
    raw_root = profile.get_data("mentioned_blog_ids_by_channel", {})
    by_ch: Dict[str, Any] = dict(raw_root) if isinstance(raw_root, dict) else {}

    for mt, ids in batch.items():
        prev = by_ch.get(mt)
        step_map: Dict[str, Any] = dict(prev) if isinstance(prev, dict) else {}
        step_map[step_key] = _merge_blog_ids(step_map.get(step_key), ids)
        by_ch[mt] = step_map

    profile.update_data("mentioned_blog_ids_by_channel", by_ch)
...
def save_recommended_blog_ids_by_channel_snapshot(env: Any, step_num: int) -> None:
    """Write datasets/step_N/user_recommended_blog_ids_by_channel.json."""
    ...
    env.data["user_recommended_blog_ids_by_channel"] = combined
    try:
...
def save_mentioned_blog_ids_by_channel_snapshot(env: Any, step_num: int) -> None:
    """Write datasets/step_N/user_mentioned_blog_ids_by_channel.json."""
    step_dir = os.path.join(_snapshot_base_dir(env), "datasets", f"step_{step_num}")
    os.makedirs(step_dir, exist_ok=True)
    ...
    with open(snapshot_path, "w", encoding="utf-8") as f:
        json.dump(combined, f, ensure_ascii=False, indent=2, default=str)
    env.data["user_mentioned_blog_ids_by_channel"] = combined
    logger.info(
...
def save_channel_snapshots(env: Any, step_num: int) -> None:
    ...
    save_recommended_blog_ids_by_channel_snapshot(env, step_num)
    save_mentioned_blog_ids_by_channel_snapshot(env, step_num)
