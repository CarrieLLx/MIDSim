# -*- coding: utf-8 -*-
"""Text diversity (online metric): TTR, Distinct-2/3, 1-Self-BLEU, Div_sem for Twitter quote/reply."""
...
try:
    from .embedding_metrics import (
        ...
        embed_content_pool_propagations,
...
def compute_diversity_metrics(
    ...
    *,
    ...
    cleaned = [t.strip() for t in texts if (t or "").strip()]
    if not cleaned:
        raise ValueError("no valid propagation texts")
    ...
    return {
        ...
        "div_sem": div_sem,
        "n_propagations": float(len(cleaned)),
    }
...
def calculate_text_diversity(data: Dict[str, Any]) -> Any:
    ...
    empty: Dict[str, Any] = {
        "_viz_kind": "propagation_diversity",
        "ttr": None,
        "distinct_2": None,
        "distinct_3": None,
    ...
    try:
        ...
        records, _cache = embed_content_pool_propagations(content_pool, data)
        if not records:
            return empty
