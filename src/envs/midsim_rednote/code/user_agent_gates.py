...
class MemorySimilarityGate:
    ...
    KW_COACHING = (
        "\n\n【话题与 memory — 关键词重叠】\n            "
        "- 当前批次与已存记忆在关键词层面判定为显著相关 → **强烈倾向 comment=false**"
        "（易与 memory 中已有表态或同题讨论重复；仅当步骤1.5 明确满足可核验新信息与强动机等破例条件时再考虑 comment=true）；"
    )
    EMB_COACHING = (
        "\n\n【话题与 memory — 语义相似】\n            "
        "- 向量相似度达到设定阈值，本批话题与记忆中内容相近 → **强烈倾向 comment=false**"
        "（视同同脉络/易重复话题，须严格按步骤1.5 评估是否仍 comment=true）；"
    )
    STRICT_SIM_COACHING = """
                ...
                """
    # iclr_rec llama31: stricter batch-wide silence when low activity + embedding overlap
    LLAMA_STRICT_SIM_COACHING = """
                步骤1.5：本批与 memory 重叠时，decisions 中全部 comment=false，不得对任一条 note 评论。
                - 默认 comment=false。若当前内容与 memory 指向**同一事件/同一问题/同一争议脉络**，或与 memory 中重叠的词超过1个，无论你是否**已在同类内容上转发、表态过**，则**几乎必须保持comment=false**。
                - **若要破例，须同时满足以下两项，缺一不可：**
                · **（1）可核对的新信息点**：`decision_reason` 须在**单句**内写清相对 memory、帖中**独有**且可指认的一条新增信息（须出现具体人/机构/日期/数字/规则名或链接类标识之一）；不得单独用「新细节」「新进展」「新讨论点」「又一例」「再关注」「同类再发酵」「略多一句」等空话充数。
                · **（2）强动机**：**强烈情绪动机**（同句或紧邻句须点明具体情绪落点，禁止空泛「有感触」「想说两句」）**或** **明确关系动机**（须点明为维护/帮扩/站队**具体的**互关、关注或好友，写清对象，禁止笼统「支持一下」）。
                ...
                - **memory_reflection 禁止自相矛盾**：先写「与 memory 重叠/同一话题/已讨论过」等，又用无（1）+（2）支撑的转折暗示可以评论——一律视为无效；若判定重叠或几乎 comment=false，memory_reflection 须**通篇**结论为倾向沉默或明确无新信息，不得以模糊语气自我放行。
                将上述结论简要写入 memory_reflection；不转发时 decision_reason 须点明「与 memory 重叠/已表态/无新信息/缺新信息或缺强动机」等。
                若本批内容与 memory 属同一事件/话题（含系统判定的关键词或语义重叠），则本批所有 note 一律 comment=false，禁止任何一条开评。
                """
    _STOPWORDS: Set[str] = {
    ...
    @staticmethod
    def topic_text_from_notes_chunk(chunk: Dict[str, Dict[str, Any]]) -> str:
        parts: list[str] = []
        for note in (chunk or {}).values():
            if not isinstance(note, dict):
                continue
            parts.append(str(note.get("title", "") or ""))
            parts.append(str(note.get("desc", "") or ""))
    ...
    async def memory_prompt_coaching(
        ...
        *,
        ...
        """Return (memory_coaching, memory_rec, memory_ref); strict step 1.5 needs low activity + embedding hit."""
        ...
        if activity < threshold and embedding_hit:
            ...
            return (
                coaching,
                "0. 未落入步骤1.5 的「几乎确定comment=false」情形，或你已写出可核验的新增信息点（仅换说法不算）；",
                "2-3句。有同题时说明是否重叠、是否几乎应comment=false",
            )
        if context == "mention":
            memory_rec = (
                "0. 未落入步骤1.5 的「几乎确定comment=false」情形，"
                "或你已写出可感知的新事实、新角度或新推理（仅换说法不算）"
            )
        else:
            memory_rec = "0. 未落入步骤1.5 的情形，或你已写出可核验的新增信息点（仅换说法不算）"
        return (
            "",
            memory_rec,
            "1-2句。无已存储记忆可对照时写「无相关记忆/首次接触」即可",
        )
...
class InteractionThreshold:
    """Sample per-round comment budget."""

    _DEFAULT_SAME = ([1, 2, 3, 4], [0.8, 0.1, 0.07, 0.03])
    _DEFAULT_DIFF = ([1, 2, 3, 4, 5], [0.7, 0.2, 0.08, 0.02, 0.01])
    _DEFAULT_KEEP = ([0, 1], [0.9784, 0.0252])
    ...
    def sample(cls, rng: random.Random) -> "InteractionThreshold":
        ...
        gate.k_same_target = cls._draw_discrete(rng, [1, 2, 3, 4], [0.8, 0.1, 0.07, 0.03])
        gate.k_diff_targets = cls._draw_discrete(rng, [1, 2, 3, 4, 5], [0.7, 0.2, 0.08, 0.02, 0.01])
        gate.k_keep_following = cls._draw_discrete(rng, [0, 1], [0.9784, 0.0252])
        return gate
...
class UserAgentGates:
    ...
    async def build_recommendation_coaching(
        ...
        return await self._build(
            chunk,
            current_timestamp,
            topic_text=MemorySimilarityGate.topic_text_from_notes_chunk(chunk),
    ...
    async def _build(
        ...
        *,
        ...
        kw_coaching, emb_coaching, emb_hit = await self.memory_similarity.run(topic_text)
