...
- 离线：默认从 SimEnv 导出的 ``content_pool_snapshot.json`` 读取各 note 下 ``comments`` 文本，
  对每条评论算与对应 note embedding 的余弦相似度，并按 note_id 输出统计表。
  可选 ``--from-llm-output-json`` 使用旧逻辑（从 LLM output JSON + comments_to_csv 解析）。
...
# 仅 --from-llm-output-json 时复用 comments_to_csv
...
def load_content_pool_from_snapshot_file(snapshot_path: str) -> Dict[str, Any]:
    ...
    支持两种顶层结构：
    - ``{ note_id: { ..., \"comments\": { ... } }, ... }``（常见）
    - ``{ \"content_pool\": { ... } }``（若外层包了 content_pool 键）
    """
...
def load_generated_comments_from_content_pool_snapshot(snapshot_path: str) -> List[Tuple[str, str]]:
    """从 ``content_pool_snapshot.json`` 提取 ``(note_id, comment_text)`` 列表。"""
    pool = load_content_pool_from_snapshot_file(snapshot_path)
    return build_generated_comments_from_content_pool(pool)
...
def run_offline_evaluation(
    ...
    *,
    ...
    2) 从 ``content_pool_snapshot.json``（默认）或 LLM output JSON 提取生成评论
    3) 计算逐评论 cosine
    4) 聚合为 note 级统计

    参数:
    - ``comments_source_path``: 默认视为 ``content_pool_snapshot.json``；若 ``from_llm_output_json=True`` 则为旧版 output JSON。
    ...
    if not os.path.isfile(comments_source_path):
        raise FileNotFoundError(f"评论来源文件不存在: {comments_source_path}")
    ...
    if from_llm_output_json:
        generated_comments = load_generated_comments_from_output(comments_source_path)
    else:
        generated_comments = load_generated_comments_from_content_pool_snapshot(comments_source_path)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-table",
        default="",
        help="按 note 聚合结果 CSV（默认: 与 content_pool_snapshot 同目录下 note_comment_similarity.csv）",
    )
