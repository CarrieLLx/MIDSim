...
def _merge_note_ids(prev_ids: Any, new_ids: List[str]) -> List[str]:
...
def record_recommendations_by_source_step(
    ...
    """Append note ids to profile.recommended_note_ids_by_channel for this source + step."""
    if profile is None:
        return
    ...
    raw_root = profile.get_data("recommended_note_ids_by_channel", {})
    ...
    merged = _merge_note_ids(step_map.get(step_key), list(recommendations.keys()))
    step_map[step_key] = merged
    by_ch[st] = step_map
    profile.update_data("recommended_note_ids_by_channel", by_ch)
...
def record_mentioned_note_ids_by_channel(
    profile: Any,
    profile_id: Any,
    current_step: int,
    ...
    """Append note ids to profile.mentioned_note_ids_by_channel for this mention type + step."""
    ...
    raw_root = profile.get_data("mentioned_note_ids_by_channel", {})
    by_ch: Dict[str, Any] = dict(raw_root) if isinstance(raw_root, dict) else {}

    for mt, ids in batch.items():
        prev = by_ch.get(mt)
        step_map: Dict[str, Any] = dict(prev) if isinstance(prev, dict) else {}
        step_map[step_key] = _merge_note_ids(step_map.get(step_key), ids)
        by_ch[mt] = step_map

    profile.update_data("mentioned_note_ids_by_channel", by_ch)
