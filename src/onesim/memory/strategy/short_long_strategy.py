...
class ShortLongStrategy(MemoryStrategy):
    def __init__(self, config: Dict[str, Any], model_config_name:str=None):
        ...
        if self.long_term_storage_name not in self._storage_map:
            ...
            if isinstance(long_term_cfg, dict) and long_term_cfg.get('model_config_name'):
                ...
                embedding_config_name = config.get('embedding_config_name') or 'vllm-embedding-bert'
            self._storage_map[self.long_term_storage_name] = self._create_storage({
                ...
                'model_config_name': embedding_config_name
            })
    ...
    async def retrieve(self, query, top_k=5) -> List[Any]:
        ...
        sample = [
            (getattr(m, "id", None), round(float(m.attributes.get("score", 0) or 0), 4))
    ...
    async def transfer_memories(self):
        ...
        logger.info(
            f"[ShortLongStrategy.transfer_memories] transferred={len(memories_to_transfer)}"
        )
    ...
    async def score(self, memories: List[Any], query: Any, weights: Dict[str, float]) -> List[Any]:
        ...
        for memory_item in memories:
            ...
            # log the score and the metrics
            metric_parts = []
            for mn, w in weights.items():
                v = memory_item.attributes.get(mn)
                try:
                    c = float(v) * float(w) if v is not None else None
                except (TypeError, ValueError):
                    c = None
                metric_parts.append(
                    f"{mn}(w={w},v={v},c={round(c, 4) if c is not None else 'n/a'})"
                )
            logger.info(
                f"[ShortLongStrategy.score] id={getattr(memory_item, 'id', None)} "
                f"total={round(float(score), 4)} " + " ".join(metric_parts)
            )
