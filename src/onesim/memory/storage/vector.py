...
class VectorMemoryStorage(MemoryStorage):
    def __init__(self, config):
        ...
        # 供日志展示（与 adapter / LoadBalancer 的 config_name 一致）
    ...
    def _embedding_model_log_tag(self) -> str:
        """日志前缀，例如 [vLLM] vllm-embedding-bert（与 chat 侧 [vLLM] vllm-qwen-9889 风格一致）"""
    ...
    def _coerce_embedding_to_1d(embedding) -> np.ndarray:
        """
        将各类 embedding 输出规整为 1D float32（FAISS 需要 (n, d) 中的单行维度 d）。
        vLLM/OpenAI 适配器在单条文本时仍常返回 list[list[float]]（batch 列表），
        若直接 append 会导致 np.array(embeddings) 维度错乱，FAISS add 报
        \"too many values to unpack (expected 2)\" 等错误。
        """
    ...
    async def _update_index_batch(self):
        ...
        try:
            ...
            # 完全重建索引（逐条规整，兼容历史错误存成的 batch 列表）
