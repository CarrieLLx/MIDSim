...
class BasicSimEnv:
    ...
    def _get_agent_pending_work_stats(self) -> tuple[int, int]:
        ...
        """
        pending_run_tasks = 0
        queued_agent_events = 0
        if not self.agents:
            return pending_run_tasks, queued_agent_events

        for agent_type_dict in self.agents.values():
            if not isinstance(agent_type_dict, dict):
                continue
            for agent in agent_type_dict.values():
                if agent is None:
                    continue
                if hasattr(agent, "count_pending_run_tasks"):
                    pending_run_tasks += agent.count_pending_run_tasks()
                elif hasattr(agent, "_run_subtasks"):
                    pending_run_tasks += sum(
                        1 for t in agent._run_subtasks if not t.done()
                    )
                if hasattr(agent, "has_pending_event_queue"):
                    if agent.has_pending_event_queue():
                        queued_agent_events += 1
                elif hasattr(agent, "_queue") and not agent._queue.empty():
                    queued_agent_events += 1
        return pending_run_tasks, queued_agent_events
    ...
    async def _check_round_completion(self):
        ...
        pending_run_tasks, queued_agent_events = self._get_agent_pending_work_stats()
        agents_work_idle = pending_run_tasks == 0 and queued_agent_events == 0
        bus_and_env_idle = (
            self.event_bus.is_empty() and
            self._queue.empty() and
            (busy_count == 0)
        )
        time_idle = current_time - self._last_event_time > idle_timeout
        long_idle = time_idle and bus_and_env_idle and agents_work_idle

        if time_idle and bus_and_env_idle and not agents_work_idle:
            logger.info(
                f"Step {self.current_step} idle check deferred: "
                f"pending_run_tasks={pending_run_tasks}, "
                f"agents_with_queued_events={queued_agent_events}"
            )
    ...
    async def _check_timed_completion(self):
        ...
        agents_work_idle = pending_run_tasks == 0 and queued_agent_events == 0
        long_idle = (
            current_time - self._last_event_time > self.config.bus_idle_timeout and
            self.event_bus.is_empty() and
            self._queue.empty() and
            agents_work_idle
        )
