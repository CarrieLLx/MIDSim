...
class AgentFactory:
    ...
    def load_relationship_data(self, relationship_file: str) -> Dict[str, List[Dict]]:
        ...
        with open(relationship_file, mode='r', encoding='utf-8-sig') as csvfile:
            ...
            for row in reader:
                base_relationships.append(_normalize_rel_row_keys(row))
        ...
        for tile_index in range(self.tiles_needed):
            for rel in base_relationships:
                ...
                # Support both 'source_id'/'target_id' and 'source_agent'/'target_agent' formats
                if "source_id" in rel:
                    source_base_id = str(rel["source_id"])
                    target_base_id = str(rel["target_id"])
                elif "source_agent" in rel:
                    source_base_id = str(rel["source_agent"])
                    target_base_id = str(rel["target_agent"])
                else:
                    logger.warning(f"Relationship entry missing source/target fields: {rel}")
                    continue
    ...
    def load_profiles(agent_type: str, schema: AgentSchema, json_file: str, 
        ...
        all_profile_rows = AgentFactory._normalize_profile_rows_from_json(raw)
