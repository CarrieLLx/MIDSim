...
def mem_planning_gate(locale: str, *, llama_backbone: bool = False) -> str:
    """Reaction prompt gate when Memory is non-empty. Llama+zh uses legacy llama-zh fork wording."""
    # if llama_backbone and normalize_prompt_locale(locale) == "zh":
    #     return (
    #         "### Planning 自检 · Memory 非空：若你打算 comment=true，并认为相对 ### Memory 有新信息："
    #         "请核对 **### Planning** 中【memory_overlap】【memory_novelty】【batch_comment_intent】是否与 ### Memory 一致——"
    #         "若 Planning 写 memory_overlap=yes 或 novelty 为「无」，则本批须倾向 comment=false。"
    #         "检查 ### Planning：是否只是在复述 Memory/Instruction，或充斥「很严重」「希望平台…」「值得关注」等空话？"
