...
class AgentProfile(ProfileBase):
    ...
    def get_profile(self, include_private: bool = False) -> Dict[str, Any]:
        """Return the profile as a dictionary. For UserAgent, historical_notes/history_notes are limited to first 10."""
        result = {**self._public_fields, **self._private_fields} if include_private else dict(self._public_fields)
        agent_type = result.get("agent_type") or getattr(self, "agent_type", None)
        if agent_type == "UserAgent":
            # Keep prompts compact while retaining both ends of long summaries.
            # If historical_summary is very long, keep the first/last N characters.
            summary_key = "historical_summary"
            if summary_key in result and isinstance(result.get(summary_key), str):
                s = result.get(summary_key) or ""
                head_tail_n = 50
                if len(s) > head_tail_n * 2 + 20:
                    result[summary_key] = f"{s[:head_tail_n]}...{s[-head_tail_n:]}"
            for key in ("historical_notes", "history_notes"):
