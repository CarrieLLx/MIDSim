...
class VLLMChatAdapter(ModelAdapterBase):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Limit concurrent requests to vLLM to avoid overload (not passed to OpenAI client)
        max_concurrent = int(client_args.pop("max_concurrent_requests", 8))
        if max_concurrent < 1:
            max_concurrent = 1
        self._sync_semaphore = threading.Semaphore(max_concurrent)
        self._async_semaphore = asyncio.Semaphore(max_concurrent)
        logger.info(f"vLLM adapter '{config_name}' limiting to {max_concurrent} concurrent request(s)")
